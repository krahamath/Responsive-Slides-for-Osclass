	...
	if( Params::getParam('create') == 1) {
		...
		if (in_array($_FILES["image"]["type"], $valid_mime_types)){
			...
			if(move_uploaded_file($_FILES["image"]["tmp_name"], $destination)){
				...
				<?php _e('The slider has been updated.','rslides'); ?>
				</div>
				<?php
				...
				<?php _e('There was an error uploading the file, please try again!', 'rslides'); ?>
				</div>
				<?php
			}
		}
			...
			?>
			<div class="slidererror">
			<?php _e('File type not allowed, Allowed file: *.jpg,*.jpeg,*.png,*.gif', 'rslides'); ?>
			</div>
			<?php
		}		
	}
...
<div class="rslides_menu">
	<ul>
		<li class="active"><a href="<?php echo osc_admin_render_plugin_url("responsive_slides/admin/create.php"); ?>"><?php _e('Create', 'rslides'); ?></a></li>
...
<div class="rslides_content">
	<h2 class="render-title"><?php _e('Add Slide', 'rslides'); ?></h2>
	<div class="form-horizontal">
		<form method="post" action="<?php osc_admin_base_url(true); ?>" enctype="multipart/form-data">
			...
			<fieldset>
			<div class="form-row">
				<div class="form-label"><?php _e('Image', 'rslides'); ?></div>
			...
			<div class="form-row">
				...
				<div class="form-controls">
					<input type="text" name="caption" value="" placeholder="<?php echo osc_esc_html(__('Enter caption','rslides')); ?>" class="xlarge"/>
				</div>
			</div>
			<div class="form-row">
				<div class="form-label"><?php _e('Description','rslides'); ?></div>
				<div class="form-controls">
					<textarea placeholder="Enter description" name="description" placeholder="<?php echo osc_esc_html(__('Enter description','rslides')); ?>"></textarea>
				</div>
			</div>
			<div class="form-row">
				<div class="form-label"><?php _e('Link to URL','rslides'); ?></div>
				<div class="form-controls">
					<input class="xlarge" type="text" name="link" value="" placeholder="<?php echo osc_esc_html(__('http://www.example.com','rslides')); ?>" class="xlarge"/>
				</div>
			</div>
			</fieldset>
			
			<div class="form-actions">
				<button type="submit" class="btn btn-submit"><?php _e('Save changes', 'rslides'); ?></button>
			</div>
		</form>
	</div>
</div>
