...
if( Params::getParam('actions') == 'delete' ){
	...
	<?php _e('The slide has been deleted.','rslides'); ?>
	</div>
	<?php
	...
	if( Params::getParam('update') == 1) {
		...
		if (in_array($_FILES["image"]["type"], $valid_mime_types)){
			...
			if(move_uploaded_file($_FILES["image"]["tmp_name"], $destination)){
				...
				?>
				<div class="slidersuccess">
				<?php _e('The slide has been updated.','rslides'); ?>
				</div>
				<?php
				echo "<script>location.href='".osc_admin_render_plugin_url("responsive_slides/admin/list.php")."'</script>";
			} 
...
<div class="rslides_content">
	<h2 class="render-title"><?php _e('Update Slide','rslides'); ?></h2>
	...
	<fieldset>
	<div class="form-row">
		<div class="form-label"><?php _e('Upload image', 'rslides'); ?></div>
	...
	<div class="form-row">
		<strong><?php _e('OR','rslides'); ?></strong>
	</div>
	<div class="form-row">
		<?php _e('Select a image:','rslides'); ?>
	</div>
	<div class="form-row">			
	...
	<div class="form-row">
		...
		<div class="form-controls">
			<input type="text" name="caption" value="<?php echo osc_esc_html($slidebyid['caption']); ?>" placeholder="<?php echo osc_esc_html(__('Enter caption','rslides')); ?>" class="xlarge"/>
		</div>
	</div>
	<div class="form-row">
		<div class="form-label"><?php _e('Description', 'rslides'); ?></div>
		<div class="form-controls">
		<textarea name="description" placeholder="<?php echo osc_esc_html(__('Enter description','rslides')); ?>" ><?php echo $slidebyid['description']; ?></textarea>
		</div>
	</div>
	<div class="form-row">
